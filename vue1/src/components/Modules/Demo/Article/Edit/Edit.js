...
module.exports = {
			...
			ref.validate((valid) => {
				if (valid) {
					...
					this.$$api_article_saveArticle(this.article_data, data => {
						this.$router.push('/components/article/list');
					});
				}
			});
		}
	...
	mounted(){
		...
		editor.config.uploadImgUrl = gbs.host + '/auth/imgup';
		...
		editor.config.uploadImgFns.onload = (data) => {
			if (data.status === 200) {
				...
				editor.command(null, 'insertHtml', '<img src="' + data.data.img_url + '" alt="' + originalName + '" style="max-width:100%;"/>');
			} else {
				if (data.status === 404) {
		...
		if (this.$route.query.id) {
			...
			this.$$api_article_findArticle(data, (data) => {
				...
				this.article_data.status = data.data.status == 1 ? true : false;
